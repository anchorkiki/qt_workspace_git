...
void ReplaceDialog::on_bt_next_clicked()
{

    QString target = ui->search_edit->text().trimmed(); // 去除首尾空格
    ...
    if (target.isEmpty()) { // 处理查找目标为空的情况
        ...
        ui->search_edit->setFocus(); // 光标定位到输入框
        return;
    }
    ...
    if (ui->rbDown->isChecked()) { // 向下查找
        ...
        // lastIndexOf的第二个参数改为 c.position() - 1（从光标前一个字符开始找）
...
void ReplaceDialog::on_bt_replace_clicked()
{
    QString target = ui->search_edit->text();
    QString to = ui->replace_edit->text();
...
void ReplaceDialog::on_bt_replaceAll_clicked()
{
    QString target = ui->search_edit->text();
    QString to = ui->replace_edit->text();
    if ((pTextEdit != nullptr) && (target != "") && (to != "")) {
        QString text = pTextEdit->toPlainText();
        text.replace(target, to, ui->radioButton->isChecked() ? Qt::CaseSensitive : Qt::CaseInsensitive);
        pTextEdit->clear();
...
void ReplaceDialog::on_bt_cancel_clicked()
{
    accept();
}
